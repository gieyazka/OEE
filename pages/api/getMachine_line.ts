...
export default async function handler(
    ...
    if (req.method === "GET") {
        ...
        let query = `SELECT  last("part_name") as part_name,last("actual_rate") as actual_rate ,last("target") as target ,last("planned_rate") as ct_time , last("status") as status  FROM "Performance" WHERE  ("Line" = '${(line as string)}') `;
        let data = {
            method: "post",
