...
export default async function handler(
    ...
    if (req.method === "GET") {
        ...
        let queryOee = `SELECT SUM(A) * SUM(P) FROM (SELECT SUM("runTime") / SUM("totalTime") as A FROM (SELECT last("execute")  -(- last("idle"))  AS runTime , last("execute") -(- last("idle"))  -(- last("stopped"))  as totalTime FROM "Performance" WHERE ${where} GROUP BY time(1s) fill(previous))) , (SELECT last(accumulator) / SUM(Runtime) as P FROM (SELECT SUM("acc") as accumulator FROM (SELECT last("accumulator") as acc FROM "Performance" WHERE accumulator > 0 AND ${where}  GROUP BY time(10s) fill(none))) , (SELECT (SUM("totalTime") / last("cycleTime")) as Runtime FROM (SELECT last("execute")  - (-last("idle")) as totalTime , last("planned_rate")  as cycleTime FROM "Performance" WHERE ${where} GROUP BY time(1s) fill(previous))))
        `;
